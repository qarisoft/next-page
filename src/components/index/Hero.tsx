  ...
  return (
    <div className=" h-screen relative mt-[-4rem] top-0 left-0 w-full overflow-hidden">
      <div
        ...
        style={{ backgroundImage: "url('./image/11.jpg')" }}
      ></div>
      {images.map((i, index) => (
        <div key={index}>
          <div className=" ">
            ...
            <div
              className="h-full w-full absolute top-0 left-0 bg-gray-800 opacity-25 overlay-color"
              style={{ backgroundColor: "black", zIndex: 3, opacity: 0.1 }}
            ></div>
          </div>
          ...
          <div className=" absolute w-full h-full  flex justify-center  items-center slide-c ">
            <Item
              data={i}
              className={`animate__animate ${index == activeI ? "fdInDown" : ""}`}
              c={index == activeI ? "nh" : "hidden"}
              n={activeI}
            />
          </div>
...
function Item({ className, c, data, n }: Props) {
  return (
    <div className={`  max-w-3xl px-2    ${c} sc `}>
      ...
      <h1
        className={`text-[#f0cb45] text-4xl font-bold   ad-1 ${className} ${!data.title && "hidden"}`}
      >
        احدث مشاريعنا
      </h1>
      ...
      <h1
        className={`text-[#d7d7d7] text-5xl md:text-8xl font-bold mb-5  ad-2 ${className}`}
      >
        {data.title}
      </h1>
      ...
      <div
        className={`bg-white w-fit rounded-md mt-2   p-2 ad-4  ${className} ${!data.title && "hidden"}`}
      >
